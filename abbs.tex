...
\newcommand{\changed}[1]{{\color{DodgerBlue}#1}}
...
Virtually all common 2D experiments, such as HSQC, HMQC, COSY, TOCSY, and NOESY, have been implemented in NOAH supersequences, allowing for the (manual or computer-assisted) structural elucidation of a wide range of molecules.\autocite{Kupce2018CC,Kupce2019JMR,Kupce2021JACSA}
...
In principle, a similar interleaving may be performed for \textit{all} modules in a supersequence.
However, it is important to remember that earlier modules affect the amount of magnetisation passed on to the later modules; thus, it is typically more robust to interleave later modules in a sequence, which avoids discrepancies in data intensity or spectral quality.
}
...
This particular ratio (with $n = 2$) yielded the spectra shown in \cref{fig:abbs}.
The pulse programmes provided in the \SInf{} encode these ratios as user-defined constants which may be customised as desired.
}
...
This is most easily accomplished by simply reducing the number of transients for the \nitrogen{} HMBC by $n$ and diverting these instead towards a \nitrogen{} seHSQC, and means that the second slot in the supersequence now alternates between four different experiments (\cref{fig:sequences_abbss}).
In principle, the \nitrogen{} seHSQC uses only \magn{\ch{N}} magnetisation (i.e.\ protons directly bonded to \nitrogen{}), and can simply be added \textit{linearly} as a third module to the supersequence: such an arrangement would maximise its sensitivity as a larger number of transients are collected.
However, this would compromise the performance of the other modules, as they must then be modified to preserve the requisite \magn{\ch{N}} magnetisation: for example, the HMBC modules would need to be modified to include the $zz$-filter\autocite{Kupce2018CC,Kupce2019JMR}, which generally causes 10--20\% sensitivity losses.
As the \nitrogen{} seHSQC is a relatively undemanding experiment in terms of sensitivity, it makes more sense to implement it in a `vertical', interleaved manner.
